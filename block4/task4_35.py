...
if table_long_side < table_short_side:
    ...
    print("(Примечание: стороны стола автоматически"
          f"скорректированы: {table_long_side} x {table_short_side})")
...
if rect_long_side < rect_short_side:
    ...
    print(f"(Примечание: стороны прямоугольника автоматически скорректированы:"
          f"{rect_long_side} x {rect_short_side})")

# Проверка возможности размещения
if rect_long_side > table_long_side or rect_short_side > table_short_side:
    print("\nВнимание: Прямоугольник не помещается на"
          "стол ни при каком размещении!")
    print(f"Нужно, чтобы {rect_long_side} ≤ {table_long_side}"
          f"И {rect_short_side} ≤ {table_short_side}")
else:
    # Расчет количества для каждого варианта
    # Вариант 1: длинная сторона прямоугольника вдоль длинной стороны стола
    count_option1 = (table_long_side // rect_long_side) * \
        (table_short_side // rect_short_side)

    # Вариант 2: длинная сторона прямоугольника вдоль короткой стороны стола
    count_option2 = (table_long_side // rect_short_side) * \
        (table_short_side // rect_long_side)
    ...
    if count_option1 > count_option2:
        print("\nОтвет: Лучше размещать длинной стороной вдоль"
              "ДЛИННОЙ стороны стола.")
        print(f"(Можно разместить {count_option1} прямоугольников, "
              f"что на {count_option1 - count_option2} больше.)")
    elif count_option2 > count_option1:
        print("\nОтвет: Лучше размещать длинной стороной вдоль"
              "КОРОТКОЙ стороны стола.")
        print(f"(Можно разместить {count_option2} прямоугольников, "
              f"что на {count_option2 - count_option1} больше.)")
    else:
        print(f"\nОтвет: Оба способа одинаковы — по"
              f"{count_option1} прямоугольников.")
